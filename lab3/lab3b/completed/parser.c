...
void compileFuncDecl(void) {
  ...
  /* declare the function in the current scope and enter its scope
     so that parameters are created with the function as owner */
  declareObject(func);
...
void compileProcDecl(void) {
  ...
  /* declare and enter procedure scope so parameters attach to it */
  declareObject(proc);
...
ConstantValue* compileUnsignedConstant(void) {
  ...
  case TK_IDENT:
    eat(TK_IDENT);
    {
      Object *o = lookupObject(currentToken->string);
      if (o == NULL) error(ERR_UNDECLARED_CONSTANT, currentToken->lineNo, currentToken->colNo);
      if (o->kind != OBJ_CONSTANT) error(ERR_UNDECLARED_CONSTANT, currentToken->lineNo, currentToken->colNo);
      constValue = duplicateConstantValue(o->constAttrs->value);
    }
    break;
  case TK_CHAR:
    eat(TK_CHAR);
